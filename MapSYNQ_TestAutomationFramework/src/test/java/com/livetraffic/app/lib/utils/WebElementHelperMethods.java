...
public abstract class WebElementHelperMethods extends WebElementCheck {
	...
	/**
	 * This method clicks on the element when he given id
	 * 
	 * @param id
	 *            - xpath
	 */
	protected void switchToFrameById(final String Id) {
		itsDriver.switchTo().frame(Id);
	}

	
	public String getFrameName(String attrName)
	{
		WebElement el= getDriver().findElement(By.xpath("//iframe[contains(@id,'contentIFrame')]"));
		String frameid= el.getAttribute(attrName);
		return frameid;
